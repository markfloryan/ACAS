...
import { generate_svg } from '@/assets/graph_node_svg_edit.js';
import {
  edges,
  nodes as nodeData,
  options,
...
export default {
  ...
  methods: {
        ...
        this.nodes.forEach(node => {
          ...
          if (node.id == pk) {
            ...
            if (Number.isInteger(pk) | (pk == 'saved')) {
              axios
                .delete(`${API_URL}/topics/${pk}/`, { headers: { Authorization: `Bearer ${this.profile.id_token}` } })
                .then(data => {
                  //on success print delete
                  console.log('deleted');
                  const profile = JSON.parse(localStorage.getItem('profile'));
                  axios
                    .get(`${API_URL}/topic/topics/`, { headers: { Authorization: `Bearer ${this.profile.id_token}` } })
                    .then(data => {
    ...
    async getData(arrayOfString){ // gets topics for the specific course
      ...
      let idtoPK = {};
      const profile = JSON.parse(localStorage.getItem('profile'));
      axios.get(`${API_URL}/topics/?courseId=${this.id}/`, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
        .then(data => {
    ...
    async setupTopics(arrayOfString){
      ...
      for(i=0;i<arrayOfString.length;i++){
        ...
        if(arrayOfString[i]=='category'){
          ...
          const profile = JSON.parse(localStorage.getItem('profile'));
          axios
            .post(`${API_URL}/topic/category/`, newData, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
            .then(response => {})
            .catch(()=>{
    ...
    async createGrade(id_val,Json){
      ...
      axios
      //get topics
        .get(`${API_URL}/student/topics/${this.id}/${student_id}/`, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
        .then(studentTopicData => {
          ...
          for(let x in studentTopicData.data.result){
            ...
            if(studentTopicData.data.result[x].topic.name===Json.topicName){
              ...
              axios
                .get(`${API_URL}/topic/category/${studentTopicData.data.result[x].topic.id}/${Json.category}/`, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
                .then(topictocat => {                  let exists=false;
                  for(let x in topictocat.data.result){
                    ...
                    if(topictocat.data.result.category===Json.category){
                      ...
                      axios
                        .post(`${API_URL}/grades/`, newGrade, { headers: { Authorization: `Bearer ${this.profile.id_token}` } })
                        .then(response => {})
                        .catch(()=> {
                          this.Errors.push('Grade not created for  '+Json.studentEmail+' ? '+Json.name);
                          console.log(error);
    ...
    async saveEdges2(newEdges,nametoPK) {
      let idtoPK = {};
      const profile = JSON.parse(localStorage.getItem('profile'));
      axios.get(`${API_URL}/topics/`, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
        .then(data => {
          ...
          //Posts edges to DB
          axios
            .post(`${API_URL}/topic/topics/`, edgyPost, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
            .then(data => {
              this.isSaved = true;
            })
            .catch(error => {
    ...
    async saveEdges(newEdges) {
      ...
      const profile = JSON.parse(localStorage.getItem('profile'));
      axios
        .get(`${API_URL}/topics/`, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
        .then(data => {
          //map topic names to pk values and then id values to pk values.
          //old nodes ids and pk values are the same so it only really matters for the new ones
          let tops = data.data.result.filter((value, index, arr) => {
    ...
    saveEdits() {
      return new Promise((resolve, reject) => {
        ...
        if(newNodes.length>0){
          axios
            .post(`${API_URL}/topics/`, newNodes, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
            .then(data => {
                    ...
                    .then(res => {
                      ...
                      axios
                        .post(`${API_URL}/student/topics/`, {
                          student: student_id,
                          topics: newSTT,
                        },  { headers: { Authorization: `Bearer ${this.profile.id_token}` } })
                        .then(data => {
                          console.log(data);
                        })
                        .catch(error => {
                      ...
                      // Actually Post edges
                      axios
                        .post(`${API_URL}/topic/topics/`, edgyPost,  { headers: { Authorization: `Bearer ${this.profile.id_token}` } })
                        .then(data => {
                          console.log(data);
