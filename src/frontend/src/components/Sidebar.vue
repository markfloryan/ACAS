...
export default {
  ...
  created() {
    ...
    axios.get(`${API_URL}/student/course/`,  { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
      .then((data) => {
  ...
  methods: {
    ...
    actuallyDelete(id){
      ...
      axios.delete(`${API_URL}/courses/${id}/`,  { headers: { Authorization: `Bearer ${this.profile.id_token}` } })
        .then((response) => {      
          this.openToast();
          this.setToastInfo({
            type: 'success',
