...
export default {
  ...
  methods: {
    ...
    retrieveResources() {
      ...
      axios.get(`${API_URL}/resources/${this.topicId}/`, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
        .then((resources) => {
    ...
    addResource(){
      if(!this.newResource.name || !this.newResource.link){
        ...
        const profile = JSON.parse(localStorage.getItem('profile'));
        axios.post(`${API_URL}/resources/`, resourceData, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
          .then((response) => {
            this.openToast();
            this.setToastInfo({
              type: 'success',
    ...
    removeResource(n) {
      ...
      axios.delete(`${API_URL}/resources/${this.resources[n].pk}/`, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
        .then((response) => {      
          this.openToast();
          this.setToastInfo({
            type: 'success',
