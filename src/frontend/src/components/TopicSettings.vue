<template>
  <div class="main">
    ...
    <h1>Topic Lock</h1>
    <p>You may lock this topic to make it unavailable to students or unlock it to make it available again.</p>
    <template v-if="!data.topic.locked">
      <button
        type="button"
        class="btn btn-create"
        @click="lockTopic()"
...
export default {
  ...
  methods: {
      ...
      for (let index = 0; index < this.form.weight.length; index++) {
        ...
        axios
          .put(
            ...
            this.ancestors[index], { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } }
          )
          // Do nothing
          .then(res => {})
    ...
    retrieveCategories() {
      const profile = JSON.parse(localStorage.getItem('profile'));
      let allCategories = [];
      axios.get(`${API_URL}/categories/`, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } }).then(categories => {
    ...
    getTopicAncestors() {
      const profile = JSON.parse(localStorage.getItem('profile'));
      axios
        .get(`${API_URL}/topic/topics/${this.data.topic.id}/`, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
        .then(res => {
    ...
    updateTopicWeight() {
      ...
      let data = {
        ...
        locked: this.data.topic.locked,
      };
      axios
        .put(`${API_URL}/topics/${this.data.topic.id}/`, data, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
        .then(response => {
          this.getTopicWeight();
          // Let them know things changed
          this.openToast();
    ...
    getTopicWeight() {
      const profile = JSON.parse(localStorage.getItem('profile'));
      axios
        .get(`${API_URL}/topics/${this.data.topic.id}/`, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
        .then(topic => {
    ...
    // Unlock the topic so it's visible to students
    unlockTopic() {
      const profile = JSON.parse(localStorage.getItem('profile'));
      let data = {
        name: this.data.topic.name,
        course: this.data.topic.course,
        ancestor_weight: this.data.topic.weight,
        locked: false,
      };
      axios
        .put(`${API_URL}/topics/${this.data.topic.id}/`, data, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
        .then(response => {
          ...
          this.setToastInfo({
            ...
            message: 'The topic was successfully unlocked',
            duration: 6000,
          });
          this.data.topic.locked = false;
        })
        .catch(error => {
          console.log(error);
          this.openToast();
          this.setToastInfo({
            type: 'error',
            title: 'Could not unlock topic',
            message: 'Please try again',
            duration: 6000,
          });
        });
    },
    // Lock the topic so it's not visible to students
    lockTopic() {
      const profile = JSON.parse(localStorage.getItem('profile'));
      let data = {
        name: this.data.topic.name,
      ...
      axios
        ...
        .then(response => {
          ...
          this.setToastInfo({
            type: 'success',
            title: 'Topic locked',
            message: 'The topic was successfully locked',
            duration: 6000,
          });
          this.data.topic.locked = true;
        })
        .catch(error => {
          console.log(error);
          this.openToast();
    ...
    retrieveTopicToCategories() {
      const profile = JSON.parse(localStorage.getItem('profile'));
      axios
        .get(`${API_URL}/topic/category/${this.data.topic.id}/`, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
        .then(categories => {
    ...
    addTopicToCategory() {
      ...
      if (
        ...
        console.log('cat dat : ', data);
        // Post and create
        axios
          .post(`${API_URL}/topic/category/`, data, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
          .then(response => {})
          .catch(error => {
            console.log(error);
            this.openToast();
    ...
    removeTopicToCategory(n) {
      ...
      axios
        ...
        .then(categories => {
          ...
          // Delete the resource
          axios
            .delete(`${API_URL}/topic/category/${this.topicToCategory[n].pk}/`, { headers: { Authorization: `Bearer ${profile.auth.profile.id_token}` } })
